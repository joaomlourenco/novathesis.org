...
def row(r, c):
    ...
    return (f'<div class="amb-row"><div class="amb-school">{label}'
            f'<span class="repo">{html.escape(r["repo"])}</span></div>'
...
def main():
    for lang in ('en', 'pt'):
        ...
        out = (f'<!DOCTYPE html>\n<html lang="{lang}">\n<head>\n<meta charset="utf-8">\n'
               ...
               f'<div class="sec"><div class="sec-hd"><h2>{c["duties_h"]}</h2></div>'
               f'<div class="cards3">{duties}</div></div>\n'
               f'<div class="sec"><div class="note"><div><h3>{c["not_h"]}</h3>'
               f'<p style="margin-top:8px">{c["not_p"]}</p></div></div></div>\n'
               f'<div class="sec"><div class="dark"><div><h2>{c["join_h"]}</h2>'
               f'<p style="margin-top:10px">{c["join_p"]}</p></div>'
               f'<a class="btn btn-p" href="{ISSUE}">{c["join_btn"]}</a></div></div>\n'
               f'<div class="sec"><div class="sec-hd"><h2>{c["roll_h"]}</h2></div>'
               f'<p class="amb-note">{c["roll_p"]}</p>{groups}</div>\n'
               f'</main>\n{footer}\n</body>\n</html>\n')
